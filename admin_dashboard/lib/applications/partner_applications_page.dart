                  ...
                  return Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Aucune candidature de partenaire',
                          style: TextStyle(
                            fontSize: 18,
...
class _ProfessionalApplicationsTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ProfessionalDataTable(
      ...
      buildRow: (item) {
        // Get company name or fallback to name
...
class _ActionButtons extends StatelessWidget {
  ...
  Future<void> _approveApplication(BuildContext context, String applicationId) async {
    try {
      await firestoreService.approvePartnerApplication(applicationId);
      if (context.mounted) {
  ...
  Future<void> _rejectApplication(BuildContext context, String applicationId) async {
    try {
      await firestoreService.rejectPartnerApplication(applicationId);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              _DetailRow(
                'Entreprise',
                data['companyName']?.toString() ?? data['name']?.toString() ?? 'N/A',
              ),
