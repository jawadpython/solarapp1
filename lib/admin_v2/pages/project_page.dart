...
/// Project Requests Page with full features
...
class _ProjectPageState extends State<ProjectPage> {
  ...
  Future<void> _deleteRequest(BuildContext context, Map<String, dynamic> item) async {
    ...
    final name = item['name']?.toString() ?? item['fullName']?.toString() ?? 'cette demande';
            ...
            children: [
              ...
              StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  return ExportBar(
                    data: filteredData,
                    columns: const ['Date', 'Nom', 'Téléphone', 'Ville', 'Type Projet', 'Consommation', 'Statut'],
                    title: 'Études & Projets',
                    fileName: 'project_requests',
                  );
                },
              ),
              const SizedBox(height: 20),
                ...
                child: StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    return SimpleDataTable(
                      data: filteredData,
                      columns: const ['Date', 'Nom', 'Téléphone', 'Ville', 'Type Projet', 'Consommation', 'Statut', 'Actions'],
                        ...
                        return Row(
                          children: [
                            ...
                            Expanded(
                              flex: 2,
                              child: Text(
                                item['name']?.toString() ?? item['fullName']?.toString() ?? 'N/A',
                            ...
                            Expanded(
                              flex: 2,
                              child: Text(item['projectType']?.toString() ?? 'N/A'),
                            ),
                            Expanded(
                              flex: 2,
                              child: Text('${item['consumption']?.toString() ?? 'N/A'} ${item['isKwh'] == true ? 'kWh' : 'kW'}'),
                            ),
                            Expanded(
                              flex: 2,
