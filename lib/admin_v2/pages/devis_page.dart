...
/// Devis Requests Page with full features
...
class _DevisPageState extends State<DevisPage> {
  ...
  Future<void> _deleteRequest(BuildContext context, Map<String, dynamic> item) async {
    ...
    final name = item['fullName']?.toString() ?? item['name']?.toString() ?? 'cette demande';
            ...
            children: [
              ...
              StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  return ExportBar(
                    data: filteredData,
                    columns: const ['Date', 'Nom', 'Téléphone', 'Ville', 'Type Système', 'Puissance', 'Panneaux', 'Statut'],
                    title: 'Demandes de Devis',
                    fileName: 'devis_requests',
                  );
                },
              ),
              const SizedBox(height: 20),
                ...
                child: StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    return SimpleDataTable(
                      data: filteredData,
                      columns: const ['Date', 'Nom', 'Téléphone', 'Ville', 'Type Système', 'Puissance', 'Panneaux', 'Statut', 'Actions'],
                        ...
                        return Row(
                          children: [
                            Expanded(
                              flex: 2,
                              child: Text(
                                DateFormatter.formatTimestamp(item['createdAt'] ?? item['date']),
                            ...
                            Expanded(
                              flex: 2,
                              child: Text(
                                item['fullName']?.toString() ?? item['name']?.toString() ?? 'N/A',
                                style: const TextStyle(fontWeight: FontWeight.w500),
                              ),
                            ),
                            Expanded(
                              flex: 2,
                              child: Text(item['phone']?.toString() ?? item['phoneNumber']?.toString() ?? 'N/A'),
                            ),
                            Expanded(
                              flex: 2,
                            ...
                            Expanded(
                              flex: 2,
                              child: Text(item['systemType']?.toString() ?? 'N/A'),
                            ),
                            Expanded(
                              flex: 2,
                              child: Text(item['powerKW']?.toString() ?? 'N/A'),
                            ),
                            Expanded(
                              flex: 2,
