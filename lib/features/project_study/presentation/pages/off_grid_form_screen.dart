...
class OffGridFormScreen extends StatefulWidget {
  const OffGridFormScreen({super.key});

  @override
  State<OffGridFormScreen> createState() => _OffGridFormScreenState();
}

class _OffGridFormScreenState extends State<OffGridFormScreen> {
  ...
  final _autonomyDaysController = TextEditingController(text: '2');
  
  int _selectedPanelPower = 550;
  int _batteryVoltage = 12;
  double _pvPower = 0;
  ...
  void dispose() {
    ...
    _autonomyDaysController.dispose();
    super.dispose();
  }

  void _calculate() {
    final dailyConsumption = double.tryParse(_dailyConsumptionController.text) ?? 0;
    final days = int.tryParse(_autonomyDaysController.text) ?? 2;

    // Safe division by zero check
    if (dailyConsumption > 0 && days > 0 && _defaultSunHours > 0 && _selectedPanelPower > 0) {
      // OFF-GRID Formula: BatteryCapacity = dailyConsumption * days
      _batteryCapacity = dailyConsumption * days;
      
      // PvKw = dailyConsumption / SunHours
      _pvPower = dailyConsumption / _defaultSunHours;
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              TextFormField(
                controller: _autonomyDaysController,
                keyboardType: TextInputType.number,
                onChanged: (_) => _calculate(),
                validator: (value) {
                  if (value == null || value.isEmpty) {
                    return 'Veuillez entrer les jours d\'autonomie';
                  }
              ...
              Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<int>(
                    ...
                    items: _batteryVoltages.map((voltage) {
